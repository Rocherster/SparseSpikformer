        ...
        if args.pruning_scope == 'local':
            ...
            for name, param in model.named_parameters():
                ...
                if 'weight' in name:
                    ...
                    # Convert Tensors to numpy and calculate
                    weight_dev = param.device
                    new_mask = np.where(abs(tensor) < percentile_value, 0, mask[step])  # 小于percentile_value的部分被剪枝 
...
def prune_by_random(args, percent, mask, model):
    ...
    for name, param in model.named_parameters():
        ...
        if 'weight' in name:
            ...
            alive = tensor[np.nonzero(tensor)]  # flattened array of nonzero values
            all_param.append(list(abs(alive)))
    avg = np.mean(np.concatenate(all_param))
    ...
    for name, param in model.named_parameters():
        ...
        if 'weight' in name:
            ...
            # Convert Tensors to numpy and calculate
            weight_dev = param.device
            normalized_weight = abs(tensor) / avg
            rand_mask = np.random.uniform(size=normalized_weight.shape)
            new_mask = np.where(rand_mask <= prune_percent/100, 0, 1) * mask[step]

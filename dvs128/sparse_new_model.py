    ...
    def forward(self, x):
        ...
        for i, blk in enumerate(block):
            if i in self.pruning_loc: 
                ...
                spatial_x = spatial_x.transpose(-2,-1) # B,N,C
                # spatial_x = x[:, 1:]
...
def spikformer(pretrained=False, **kwargs):
    model = SpikformerPrune(
        ...
        pruning_loc = [1,2,3],   # pruning_loc = [1,2,3]
        token_ratio = [0.5, 0.5**2, 0.5**3],  # token_ratio = [0.8 , 0.8**2, 0.8**3]
        distill = True,
        **kwargs
    )
...
def spikformerteacher(pretrained=False, **kwargs):
    model = SpikformerTeacher(
        patch_size=16, embed_dims=256, num_heads=16, mlp_ratios=4,
        in_channels=2, num_classes=11, qkv_bias=False,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=4, sr_ratios=1,   # depths : 4->2
        **kwargs
    )
    model.default_cfg = _cfg()
    return model

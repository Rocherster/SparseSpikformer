...
def parse_args():
    ...
    parser.add_argument('--model', default='Sparse_Spikformer_new', help='model')
    ...
    parser.add_argument('--output-dir', default='./logs/Sparse_new_lth', help='path where to save')
...
def main(args):
    ...
    pretrained = torch.load('/home/ly22216501/SNN/transformer/SparseSpikformer/dvs128-gesture/pretrained/checkpoint_max_test_acc1.pth', map_location='cpu')

    #model.load_state_dict(pretrained['model'])
    #teacher_model.load_state_dict(pretrained['model'])
    ...
    # torch.save(model.state_dict(), f"{os.getcwd()}/sparse_new_snn_laterewind_lth/sparse_initial_state_dict.pth.tar")

    initial_state_dict = torch.load("/home/ly22216501/SNN/transformer/SparseSpikformer/dvs128-gesture/sparse_snn_laterewind_lth/sparse_laterewinding_state_dict.pth.tar")

    checkpoint = torch.load("/home/ly22216501/SNN/transformer/SparseSpikformer/dvs128-gesture/logs/Sparse_lth/Sparse_Spikformer_new_b16_T16_wd0.06_adamw_cnf_ADD/lr0.001_Iteration11/checkpoint_299.pth")
    model.load_state_dict(checkpoint['model'])

    # Making Initial Mask
    # mask = make_mask(model)
    with open(f"{os.getcwd()}/dumps/sparse_new_snn_laterewind_lth/mask_5.2.pkl", 'rb') as fp:  # 加载mask 保证和 lth使用相同的mask
             mask = pickle.load(fp)
    ...
    for _ite in range(12, ITERATION):
        ...
        for epoch in range(args.start_epoch, args.end_iter):
            ...
            if _ite == 0 and epoch == args.rewinding_epoch:  # rewinding_epoch=20
                ...
                utl.checkdir(f"{os.getcwd()}/sparse_snn_laterewind_lth/")
                torch.save(initial_state_dict, f"{os.getcwd()}/sparse_snn_laterewind_lth/sparse_laterewinding_state_dict.pth.tar")

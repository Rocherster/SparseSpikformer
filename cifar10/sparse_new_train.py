...
import sparse_new_model
from sparse_new_model import SpikformerTeacher
from sparse_new_model import SpikformerPrune
try:
...
# Model detail
parser.add_argument('--model', default='sparse_spikformer_cifar10', type=str, metavar='MODEL',
...
def main():
    ...
    args, args_text = _parse_args()  # 解析yaml文件
    ...
    SPARSE_RATIO = [args.base_rate, args.base_rate - 0.2, args.base_rate - 0.4]   # sparse ratio[0.9 0.7 0.5]
    KEEP_RATE = [SPARSE_RATIO[0], SPARSE_RATIO[0] ** 2, SPARSE_RATIO[0] ** 3]     # rate[0.9 0.81 0.729]
    ...
    # teacher_model = SpikformerTeacher(
    #     drop_rate=0.,
    #     drop_path_rate=0.,
    #     img_size_h=args.img_size, img_size_w=args.img_size,
    #     patch_size=args.patch_size, embed_dims=args.dim, num_heads=args.num_heads, mlp_ratios=args.mlp_ratio,
    ...
    pretrained = torch.load('./output/train/20230408-130943-spikformer-32/model_best.pth.tar', map_location='cpu')
    ...
    data_config = resolve_data_config(vars(args), model=model, verbose=args.local_rank == 0)  # timm
    ...
    train_loss_fn = DistillDiffPruningLoss_dynamic(
            teacher_model, train_loss_fn, clf_weight=1.0, keep_ratio=KEEP_RATE, mse_token=True, ratio_weight=args.ratio_weight, distill_weight=0.5
        )

    # setup checkpoint saver and eval metric tracking
    eval_metric = args.eval_metric  # top1
    best_metric = None
    best_epoch = None
    saver = None
    output_dir = None
    if args.rank == 0:
        ...
        output_dir = get_outdir(args.output if args.output else './output/sparse_new_baserate_' + str(args.base_rate), exp_name)
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if lr_scheduler is not None:
                ...
                lr_scheduler.step(epoch + 1, eval_metrics[eval_metric])  # eval_metrics[top1]
            ...
            if saver is not None:
                ...
                save_metric = eval_metrics[eval_metric]  # eval_metrics[top1]
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            output = model(input)
            loss,loss_part = loss_fn(input, output, target)

...
AutoAugment:从数据中学习增强策略-https://arxiv.org/abs/1805.09501

用于对象检测的学习数据增强策略-https://arxiv.org/abs/1906.11172

RandAugment:实用的自动化数据扩充…-https://arxiv.org/abs/1909.13719

AugMix:一种提高鲁棒性和不确定性的简单数据处理方法https://arxiv.org/abs/1912.02781
"""
import random
import math
import re
...
_PIL_VER = tuple([int(x) for x in PIL.__version__.split('.')[:2]])  #(6,2)
...
_HPARAMS_DEFAULT = dict(
    translate_const=250,
    img_mean=_FILL,
)  # {'translate_const': 250, 'img_mean': (128, 128, 128)}

_RANDOM_INTERPOLATION = (Image.BILINEAR, Image.BICUBIC)   # (2,3)
...
def rand_augment_transform(config_str, hparams):   ####
    ...
    param config_str：定义随机扩充配置的字符串。由多个部分组成，由

    破折号（'-'）。第一节定义了rand扩增的具体变体（目前只有“rand”）。剩下的

    节，而不是顺序特定确定

    “m”-rand增量的整数幅度

    “n”-整数num层（每个图像选择的变换操作数）

    “w”-整数概率权重指数（影响操作选择的一组权重的指数）

    “mstd”-应用的幅度噪声的浮点标准偏差，或在无穷大（或>100）时进行均匀采样

    “mmax”-将幅值上限设置为默认值_LEVEL_DENOM（10）以外的值

    “inc”-整数（bool），使用严重性随大小增加的增强（默认值：0）

    Ex“rand-m9-n3-mstd0.5”导致RandAugment的震级为9，num_layers 3，magnitude_std 0.5

    “rand-mstd1-w0”导致magnitude_std 1.0，权重0，默认值为10，num_layers 2



    ：param hparams:RandAugmentation方案的其他hparams（kwargs）
